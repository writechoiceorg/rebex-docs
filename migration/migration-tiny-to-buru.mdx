...
Since the Tiny SFTP Server uses a simple XML configuration file, no automated export tool is available. You must manually retrieve your settings from the configuration file and apply them to the Buru version using the Web Administration dashboard.

## Prerequisites

To perform the migration, you need the following:

* Access to the `RebexTinySftpServer.exe.config` file. 
  * This file is located in the same folder as the Tiny SFTP executable.
...
<Steps>
  <Step title="Retrieve current settings">
    ...
    3.  In the configuration file, locate the `<appSettings>` section.
  ...
  <Step title="Configure Advanced Settings (Optional)">
    ...
    <Steps>
      ...
      <Step title="User Permissions and Public Keys">
        ...
        2.  Click on the **Username** you migrated (for example, "tester").
  ...
  <Step title="Verify the Connection">
    ...
    3.  Enter the **Hostname** (for example, `localhost`).
